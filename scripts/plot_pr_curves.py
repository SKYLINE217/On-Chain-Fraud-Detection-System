...
def plot_curves(entity_project: str, best_sage: str, best_gat: str):
    ...
    # For each model to include in comparison:
    model_runs = {
    ...
    for label, run_id in model_runs.items():
        ...
        try:
            # We search for run by name or ID. Usually run_id is the ID.
            # But baseline-xgb is a name. Let's handle both.
            # Easiest way to handle names is to query by name if it doesn't look like an ID.
            ...
            for art in artifacts:
                # W&B plot line artifacts usually contain pr curve data
                if "pr_curve" in art.name or art.type == "run_table":
                    # W&B UI handles tables differently, we download the raw json
                    art_dir = art.download()
                    import json
                    # The artifact contains a .json file with columns and data
                    for file in os.listdir(art_dir):
                        if file.endswith(".json"):
                            with open(os.path.join(art_dir, file), 'r') as f:
                                data = json.load(f)
                                # Extract recall and precision from table data

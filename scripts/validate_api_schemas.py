...
TEST_ADDRESS = "896630"  # known address from dataset
...
def check_wallet():
    ...
    d = r.json()
    assert all(k in d for k in ["address", "risk_score", "predicted_label", "confidence", "timeStep", "communityId"]), \
        f"Missing keys in wallet response: {d.keys()}"
    assert isinstance(d["risk_score"], (int, float)) and 0.0 <= d["risk_score"] <= 1.0, \
        f"risk_score out of range: {d['risk_score']}"
...
def check_subgraph():
    ...
    d = r.json()
    assert all(k in d for k in ["nodes", "edges", "hops", "node_count"]), \
        f"Missing keys in subgraph response: {d.keys()}"
...
def check_cluster_list():
    ...
    if d:
        assert all(k in d[0] for k in ["communityId", "size", "avg_risk", "max_risk"]), \
            f"Missing keys in cluster response: {d[0].keys()}"
    print("✅ /cluster/list")
